...
All pair testing 工具的開發：

1.  **程式技術：** 主要需要**組合數學 (Combinatorics)**、**圖論 (Graph Theory)** 的知識，以及能夠高效處理資料結構和實現**優化演算法**的程式語言（如 Java, Python, C++）。
2.  **複雜度：** 設計出能生成**最小測試案例集**的 All-Pairs 測試問題，**是 NP-hard 問題**。然而，實際使用的生成演算法（如 ACTS 中的 IPO 演算法）是高效的**貪婪啟發式 (Greedy Heuristic)** 演算法，它們通常能在合理的時間內找到**近似最佳解**。

工具：
* **PICT** (Pairwise Independent Combinatorial Testing), 容易取得，免費, Windows (命令行工具), 無原生的 Java 版本, 微軟的開源工具，非常流行，但需要 Windows 環境。
...
如果有五個因子，分別有 4,5,6,7,8 個類別，那們強涵蓋就需要 4*5*6*7*8=6720 個測試案例。太過於複雜。

**1. ✅分離例外**

* **基本原理：** 嚴格區分**有效等價類**和**無效等價類**，並在無效類中進一步識別出**例外類別**。
* **具體做法：**
    * **扣除等效的例外類別：** 如果所有因子都有**相同**的異常行為（例如，所有因子的輸入超限都會導致相同的錯誤代碼），則您不需要為每個因子測試這個異常的組合。您可以將這些異常類別視為一個**單獨的集合**，只在特定的、最能代表異常的組合中測試，或者只測試它們與有效類別的組合。
    * **最小有效集：** 對於 $N$ 個類別，如果 $N-k$ 個類別具有相同的成功/有效行為，則只需要選擇其中一個類別作為**代表**進行有效組合測試。測試數量變成 (4-2) * (5-2) ... (8-2) = 720 大量的減少測試案例。

**2. ✅配對組合**

可以改用 2-way 或是 n-way 的部分組合方式來進行測試。
...
* 缺陷最容易出現在等價類的邊界上。
* 對於每個因子的每個等價類，測試其**精確邊界**、**邊界內**一個值、**邊界外**一個值（例如：$N$, $N-1$, $N+1$）。雖然邊界值分析不會直接減少因子數量，但它**取代**了在等價類內部隨機選擇值的需求，讓選取的測試點更有針對性，從而避免了對整個等價類的冗餘測試。

**4. ✅故障模式和影響分析**

Failure Mode and Effects Analysis, FMEA

* **基本原理：** 根據每個因子可能導致的風險和影響來分配測試資源。
* **具體做法：**
